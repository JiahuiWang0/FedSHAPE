...
def train(model: FederatedModel, private_dataset: FederatedDataset,
    ...
    if args.rand_dataset:
        ...
        if model.args.dataset == 'fl_digits':
            ...
            selected_domain_dict = {'caltech': args.caltech, 'amazon': args.amazon, 'webcam': args.webcam,
                                    'dslr': args.dslr}
    ...
    model.online_clients_sequence = generate_online_clients_sequence(args.communication_epoch, args.parti_num,
                                                                     args.online_ratio)
    ...
    for epoch_index in range(Epoch):
        ...
        if model.device.type == 'cuda':
            torch.cuda.reset_peak_memory_stats(model.device)  # 重置显存峰值统计
        ...
        round_local_time = end_local_time - start_local_time

        if model.device.type == 'cuda':
            current_peak_mem = torch.cuda.max_memory_allocated(model.device) / (1024 * 1024)  # 转换为 MB
        else:
            current_peak_mem = 0.0  # CPU模式下暂不统计
        # ==========================================================
        ...
        # 打印系统开销数据到控制台
        print(f"Local Compute Time: {round_local_time:.2f} s")
        print(f"Peak VRAM Usage: {current_peak_mem:.2f} MB\n\n")

        # Save round results to CSV file
        if args.csv_log:
            csv_writer.write_round_results(epoch_index, domains_list, accs, mean_acc, std)
            # ==== 新增：将时间和显存数据写入 system_metrics.csv ====
            csv_writer.write_sys_metrics(epoch_index, round_local_time, current_peak_mem)
