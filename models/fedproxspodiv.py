...
class SPOOptim(torch.optim.Optimizer):
    ...
    def generate_delta(self, zero_grad=False):
        ...
        grad_norm = torch.norm(torch.stack(
            [(1.0 * p.grad).norm(p=2).to(device) for group in self.param_groups for p in group["params"] if
...
class FedProxSPODIV(FederatedModel):
    NAME = 'fedproxspodiv'
    COMPATIBILITY = ['homogeneity']

    def __init__(self, nets_list, args, transform):
        super(FedProxSPODIV, self).__init__(nets_list, args, transform)
    ...
    def _train_net(self, index, net, global_net, train_loader):
        ...
        optimizer = SPOOptim(net.parameters(), base_optimizer=optim.SGD, alpha=alpha, lr=self.local_lr, momentum=0.9,
                            weight_decay=1e-5)
        ...
        for _ in iterator:
            for batch_idx, (images, labels) in enumerate(train_loader):
                ...
                prox_term = sum(
                    torch.norm(w - w_t, p=2) ** 2 for w, w_t in zip(net.parameters(), global_net.parameters()))
                loss = task_loss + (self.mu / 2.0) * prox_term
                loss.backward()

                optimizer.generate_delta(zero_grad=True)

                task_loss_p = criterion(net(images), labels)
                prox_term_p = sum(
                    torch.norm(w - w_t, p=2) ** 2 for w, w_t in zip(net.parameters(), global_net.parameters()))
                loss_p = task_loss_p + (self.mu / 2.0) * prox_term_p
                loss_p.backward()
