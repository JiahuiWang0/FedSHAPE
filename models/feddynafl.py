...
class FedDynAFL(FederatedModel):
    NAME = 'feddynafl'
    COMPATIBILITY = ['homogeneity']

    def __init__(self, nets_list, args, transform):
        super(FedDynAFL, self).__init__(nets_list, args, transform)
    ...
    def ini(self):
        ...
        for i in range(self.args.parti_num):
            ...
            for name, param in self.global_net.named_parameters():
                self.local_grads[i][name] = torch.zeros_like(param.data).cpu()
    ...
    def loc_update(self, priloader_list):
        ...
        h = {}
        for name, param in self.global_net.named_parameters():
            h[name] = torch.zeros_like(param.data).to(self.device)

        for i in range(self.args.parti_num):
            weight = self.dynamic_lambdas[i].item()
            for name in h:
                h[name] += weight * self.local_grads[i][name].to(self.device)

        for name, param in self.global_net.named_parameters():
            param.data.sub_((1.0 / self.dyn_alpha) * h[name])
