...
class FedProxSHAPE(FederatedModel):
    NAME = 'fedproxshape'
    COMPATIBILITY = ['homogeneity']

    def __init__(self, nets_list, args, transform):
        super(FedProxSHAPE, self).__init__(nets_list, args, transform)
        ...
        for _ in iterator:
            for batch_idx, (images, labels) in enumerate(train_loader):
                ...
                for w, w_t in zip(net.parameters(), global_net.parameters()):
                    prox_term += torch.norm(w - w_t, p=2) ** 2
                ...
                for w_p, w_t in zip(net.parameters(), global_net.parameters()):
                    prox_term_p += torch.norm(w_p - w_t, p=2) ** 2
