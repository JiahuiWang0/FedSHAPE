...
class FedProxSPO(FederatedModel):
    NAME = 'fedproxspo'
    COMPATIBILITY = ['homogeneity']

    def __init__(self, nets_list, args, transform):
        super(FedProxSPO, self).__init__(nets_list, args, transform)
    ...
    def _train_net(self, index, net, global_net, train_loader):
        ...
        for _ in iterator:
            for batch_idx, (images, labels) in enumerate(train_loader):
                ...
                task_loss_p = criterion(outputs_p, labels)

                proximal_term_p = 0.0
                for w_p, w_t in zip(net.parameters(), global_net.parameters()):
                    proximal_term_p += torch.norm(w_p - w_t, p=2) ** 2
                proximal_term_p = (self.mu / 2.0) * proximal_term_p

                loss_p = task_loss_p + proximal_term_p
                loss_p.backward()
